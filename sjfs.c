...
void sjf_scheduler(struct Job jobs[], int n) {
    ...
    printf("Job ID\tArrival Time\tBurst Time\tWaiting Time\tCompletion Time\tTurnaround Time\n");
    ...
    for (int i = 0; i < n; i++) {
        ...
        int turnaround_time = completion_time - jobs[i].arrival_time;
        total_turnaround_time += turnaround_time;

        printf("%d\t%d\t\t%d\t\t%d\t\t%d\t\t%d\n", jobs[i].job_id, jobs[i].arrival_time, jobs[i].burst_time,
               waiting_time, completion_time, turnaround_time);
    }
